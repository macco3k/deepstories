...
class Contractions(object):
    ...
    def __init__(self, w2v_path=None, w2v_model=None, lang_code='en-US'):
        """w2v_path is a path to an embedding model used for calculating the Word Mover's Distance."""
        assert not (w2v_path is not None and w2v_model is not None), "Either word2vec path or model must be specified, but not both"  
